...
public class Ball : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D col)
    {
        if (col.gameObject.tag == "Goal 1")
        {
            ...
            GameManager.instance.SpawnBall();
            Destroy(gameObject);
            if (GameManager.instance.goldenGoal) // Fungsi ini digunakan untuk memvalidasi kondisi sedang dalam mode Golden Goal
            {
    ...
    {
        isBounce = true;
        yield return new WaitForSeconds(1f); //Kalau menggunakan ienumerator, sebagai returnnya harus menggunakan yield
        isBounce = false;
    }
}
